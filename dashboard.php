...
do {
    $ordersResp = meli_get("https://api.mercadolibre.com/orders/search?seller=$user_id&sort=date_desc&limit=$ordersLimit&offset=$ordersOffset", $access_token);
...
foreach($periodo30 as $dia){
    ...
    $data30Dias[] = round($vendas30Dias[$dia->format('Y-m-d')] ?? 0, 2);
}

// Produtos
$itemsResp = meli_get("https://api.mercadolibre.com/users/$user_id/items/search?limit=200", $access_token);
$totalProdutos = count($itemsResp['body']['results'] ?? []);
$produtosEstoque = 0;
$produtosFaltando = 0;
$produtosDetalhes = [];

if(!empty($itemsResp['body']['results'])){
    foreach($itemsResp['body']['results'] as $itemId){
...
$produtosCriticosMedios = array_filter($produtosDetalhes, function($p){
    return $p['qty'] <= $p['limit'];
});
?>
<!DOCTYPE html>
<html lang="pt-br">
...
<div class="col-6 col-md bg-white py-3 rounded shadow-sm">
<h5 class="text-primary">R$ <?= number_format($totalVendasAmount, 2, ',', '.') ?></h5>
...
<div class="col-6 col-md bg-white py-3 rounded shadow-sm">
<h5 class="text-warning">R$ <?= number_format($ticketMedio, 2, ',', '.') ?></h5>
...
<!-- Produtos críticos/médios com paginação numerada -->
...
new Chart(document.getElementById('lineChart'), {
    type:'line',
    data:{labels: <?= json_encode($labelsChart) ?>, datasets:[{label:'Vendas (R$)', data: <?= json_encode($dataChart) ?>, borderColor:'#00b894', backgroundColor:'rgba(0,184,148,0.2)', fill:true, tension:0.3}]},
    options:{responsive:true, plugins:{legend:{position:'bottom'}}}
});
new Chart(document.getElementById('lineChart30'), {
    type:'line',
    data:{labels: <?= json_encode($labels30Dias) ?>, datasets:[{label:'Vendas (R$)', data: <?= json_encode($data30Dias) ?>, borderColor:'#ff7675', backgroundColor:'rgba(255,118,117,0.2)', fill:true, tension:0.3}]},
    options:{responsive:true, plugins:{legend:{position:'bottom'}}}
});
</script>
</body>

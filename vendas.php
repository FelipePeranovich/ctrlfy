...
.table-hover tbody tr:hover { background-color: #fff3e0; cursor: pointer; transition: all 0.2s ease-in-out; }
.pagination .page-item .page-link { color: #ff6600; border-radius: 6px; transition: all 0.2s ease; }
.pagination .page-item.active .page-link { background-color: #ff6600; color: #fff; border-color: #ff6600; }
.pagination .page-item .page-link:hover { background-color: #ffc07a; color: #fff; }
...
<div class="d-flex flex-column flex-md-row">
    ...
    <div class="main-content flex-grow-1 p-4">
        ...
        if ($totalPaginas > 1) {
            ...
            if ($pagina > 1) echo '<li class="page-item"><a class="page-link" href="?pagina='.($pagina-1).'&mes='.$filtro_mes.'">&laquo;</a></li>';
            if ($inicio > 1) { 
                echo '<li class="page-item"><a class="page-link" href="?pagina=1&mes='.$filtro_mes.'">1</a></li>';
                if ($inicio > 2) echo '<li class="page-item disabled"><span class="page-link">...</span></li>';
            }
            for ($i = $inicio; $i <= $fim; $i++) {
                $active = ($i == $pagina) ? "active" : "";
                echo "<li class='page-item $active'><a class='page-link' href='?pagina=$i&mes=$filtro_mes'>$i</a></li>";
            }
            if ($fim < $totalPaginas) {
                if ($fim < $totalPaginas - 1) echo '<li class="page-item disabled"><span class="page-link">...</span></li>';
                echo '<li class="page-item"><a class="page-link" href="?pagina=' . $totalPaginas . '&mes='.$filtro_mes.'">' . $totalPaginas . '</a></li>';
            }
            if ($pagina < $totalPaginas) echo '<li class="page-item"><a class="page-link" href="?pagina=' . ($pagina + 1) . '&mes='.$filtro_mes.'">&raquo;</a></li>';
            echo '</ul></nav>';
        }
        } else {
